...
// Same form and endpoint as /contact-us; localized labels and messages.
...
export interface ContactContent {
  ...
  success: string
  failed: string
  labels: { name: string; email: string; company: string; phone: string; message: string }
  placeholders: { name: string; email: string; company: string; phone: string; message: string }
  submit: string
  submitting: string
  humans: string
  emailTitle: string
...
export default function ContactPageL10n({ locale, content: c }: { locale: L10nLocale; content: ContactContent }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || c.failed)
  ...
  return (
    <main className="relative min-h-screen bg-slate-50 grid-lines overflow-x-hidden text-slate-800 antialiased">
      ...
      <section className="max-w-6xl mx-auto px-6 pt-28 md:pt-32 pb-16 text-center space-y-6">
        ...
        <p className="text-base md:text-lg text-slate-600 font-medium max-w-3xl mx-auto leading-relaxed">{c.intro}</p>
      </section>
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
          <div className="bg-white border-2 border-slate-200 rounded-3xl p-6 sm:p-8 md:p-10 card-vivid-shadow space-y-6">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">{c.formTitle}</h2>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label htmlFor="name" className={labelClass}>{c.labels.name} *</label>
                <input id="name" name="name" type="text" required value={formData.name} onChange={handleChange} className={fieldClass} placeholder={c.placeholders.name} />
              </div>
              <div>
                <label htmlFor="email" className={labelClass}>{c.labels.email} *</label>
                <input id="email" name="email" type="email" required value={formData.email} onChange={handleChange} className={fieldClass} placeholder={c.placeholders.email} />
              </div>
              <div>
                <label htmlFor="company" className={labelClass}>{c.labels.company}</label>
                <input id="company" name="company" type="text" value={formData.company} onChange={handleChange} className={fieldClass} placeholder={c.placeholders.company} />
              </div>
              <div>
                <label htmlFor="phone" className={labelClass}>{c.labels.phone}</label>
                <input id="phone" name="phone" type="tel" value={formData.phone} onChange={handleChange} className={fieldClass} placeholder={c.placeholders.phone} />
              </div>
              <div>
                <label htmlFor="message" className={labelClass}>{c.labels.message} *</label>
                <textarea id="message" name="message" required rows={5} value={formData.message} onChange={handleChange} className={fieldClass} placeholder={c.placeholders.message} />
              </div>
              <button
                type="submit"
              ...
              >
                {isSubmitting ? <span>{c.submitting}</span> : (<><span>{c.submit}</span><Send className="w-4 h-4" /></>)}
              </button>
            </form>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-indigo-50/50 border-2 border-dashed border-indigo-200 rounded-3xl p-6 flex items-center gap-4">
              ...
              <span className="text-xs font-bold text-slate-500 leading-snug">{c.humans}</span>
            </div>
            ...
            <div className="bg-white border-2 border-slate-200 rounded-3xl p-6 sm:p-8 card-vivid-shadow">
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1">
                  <h3 className="text-base font-extrabold text-slate-900">{c.addressTitle}</h3>
                  {c.addressLines.map((l) => <p key={l} className="text-sm text-slate-600 font-medium">{l}</p>)}
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-24">
        <div className="bg-indigo-600 rounded-3xl p-12 md:p-16 text-center text-white space-y-6 shadow-xl relative overflow-hidden">
          ...
          <div className="pt-4">
            <Link href={c.ctaHref} className="px-12 py-5 bg-white hover:bg-indigo-50 text-indigo-600 font-extrabold rounded-2xl text-base transition-all shadow-md inline-block hover:scale-[1.03] active:scale-95 duration-200">
              {c.ctaButton}
            </Link>
          </div>
        </div>
