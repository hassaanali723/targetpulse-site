...
const cfg: AltPageConfig = {
  ...
  desc: 'Bounceless now resolves catch-all too, but bills 5 credits per catch-all. Giggal.ai charges 1.5, adds SEG, and is $9.90 per 10,000. 1,000 free credits, no card.',
  h1Article: 'A',
  h1Tail: 'that adds SEG verification',
  heroP:
    'Bounceless is a low-cost verifier that now resolves catch-all as a Deep add-on, but it bills 5 credits per catch-all. Giggal.ai resolves catch-all at 1.5 credits, verifies behind secure email gateways too, and is $9.90 per 10,000.',
  bluf: [
    { k: 'Catch-all', v: 'Both resolve it now. Bounceless charges 5 credits per catch-all; we charge 1.5.' },
  ...
  catchAllProse:
    'Bounceless has added Deep catch-all resolution, so like Giggal.ai it now returns a real result on catch-all domains rather than flagging them. The difference is the bill and the coverage: Bounceless charges 5 credits per catch-all, or 3 if cached, while Giggal.ai charges 1.5 in a run, and Giggal.ai also verifies addresses behind secure email gateways such as Mimecast, Proofpoint and Barracuda, which Bounceless does not.',
  pricingHeading: 'Bounceless pricing vs Giggal.ai',
  pricingProse:
    'Bounceless publishes flat pay-as-you-go tiers: $29 at 10,000, $159 at 100,000 and $579 at a million. Giggal.ai is $9.90, $76 and $680 at those volumes. On catch-all, Bounceless bills 5 credits per address and Giggal.ai bills 1.5.',
  featureNote: 'Catch-all pricing is where the two diverge: 5 credits per catch-all on Bounceless against 1.5 on Giggal.ai.',
  testStep3: 'Compare the catch-all cost, and check the gateway addresses Bounceless leaves out.',
  ctaHeadline: 'Catch-all at 1.5 credits, plus gateways',
  faqs: [
    {
      q: 'Does Bounceless resolve catch-all like Giggal.ai?',
      a: 'It does now, through a Deep catch-all add-on, so both return a real result rather than a flag. The difference is cost: Bounceless charges 5 credits per catch-all (3 if cached), while Giggal.ai charges 1.5 in a run or 2 standalone, and Giggal.ai also verifies behind secure email gateways, which Bounceless does not.',
    },
    {
      q: 'How does the pricing compare?',
      a: 'Bounceless publishes flat tiers, $29 at 10,000, $159 at 100,000 and $579 at a million. Giggal.ai is $9.90, $76 and $680 at those volumes. On catch-all, Bounceless bills 5 credits per address and Giggal.ai bills 1.5.',
    },
    {
